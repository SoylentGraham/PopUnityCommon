...
namespace PopX
{
	public static class Textures
	{
		...
		static public void ClearTexture(Texture Tex, Color Colour)
		{
			if ( Tex is Texture2D )
			{
				ClearTexture( Tex as Texture2D, Colour );
			}
			else if ( Tex is RenderTexture )
			{
				ClearTexture(Tex as RenderTexture, Colour);
			}
			else
			{
				throw new System.Exception("Unhandled texture type " + Tex.GetType().ToString());
			}
		}

		static public void ClearTexture(RenderTexture Tex, Color Colour)
		{
			//	make a 2D image to copy
			var ClearImg = new Texture2D(1, 1);
			ClearImg.SetPixel(0, 0, Colour);
			ClearImg.Apply();
			Graphics.Blit(ClearImg, Tex);
		}
